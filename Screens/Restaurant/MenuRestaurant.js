...
const MenuScreen = ({route}) => {
  ...
  useEffect(() =>{
    if (route.params?.restName) {
      ...
      setGlutineDishes([]);
      setLactoseDishes([]);
      //console.log(route.params?.restName);
      db.collection('Restaurants').get().then(doc => {
        doc.docs.map(rest => {
          if (rest.data().Name == route.params?.restName) {
            db.collection('Restaurants').doc(rest.id).collection('Branch').doc('1').collection('Category').get().then(catDish => {
              catDish.docs.map(cat => {
                db.collection('Restaurants').doc(rest.id).collection('Branch').doc('1').collection('Category').doc(cat.id).collection('Dishes').get().then(docdish => {
            ...
            db.collection('Restaurants').doc(rest.id).collection('Dishes').get().then(docDish => {
              docDish.docs.map(dish => {
                setDishes(d => [...d, dish]);
              })
            })
        ...
        return (
          ...
          <View>
            <Card.Content>
              ...
              <Paragraph style={{ marginLeft: 140,  fontSize: 17, fontWeight: "bold", marginTop: 5}}>
